...
const MessagesState = {
  ...
  get isVisible() {
    ...
    return !container?.classList.contains('hidden') && !container?.classList.contains('opacity-xl');
  },
};
...
function checkHoverState(x, y) {
  ...
  const isInMiddleArea =
    x >= middleAreaLeft && x <= middleAreaRight && y >= middleAreaTop && y <= middleAreaBottom;
...
function addMessage(role, content, options = {}) {
  const { artifactIds = null, structuredData = null, isIncremental = false } = options;
