...
class SupabaseSync {
  ...
  removeMessageFromLocalState(messageId) {
    ...
    console.log(`[SYNC] Message deletion from local state not implemented (messageId: ${messageId})`);
  }

  removeArtifactFromLocalState(artifactId) {
    console.log(`[SYNC] Removing artifact ${artifactId} from local state only`);
    const localArtifacts = (window.context?.getArtifacts() || []).filter(a => a.id !== artifactId);
  ...
  async syncMessages() {
    ...
    const localMessagesByChat = { ...(window.context?.getMessagesByChat() || {}) };
    ...
    for (const [chatId, messages] of Object.entries(localMessagesByChat)) {
      const serverMessages = serverMessagesByChat[chatId] || [];
      const localOnlyMessages = messages.filter(localMsg =>
  ...
  async syncArtifacts() {
    ...
    const localOnlyArtifacts = localArtifacts.filter(artifact => !serverArtifactIds.has(artifact.id));
